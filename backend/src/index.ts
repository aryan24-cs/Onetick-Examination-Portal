  ...
  try {
    ...
    if (adminEmail && adminPassword) {
      ...
      if (!existingAdmin) {
        ...
        await admin.save();
        console.log(
          `Admin account created for email: ${adminEmail}, hashed password: ${hashedPassword}`
        );
      } else {
        ...
        if (!isMatch) {
          console.warn(
            `Password mismatch for admin ${adminEmail}. Updating password.`
          );
          const hashedPassword = await bcrypt.hash(adminPassword, 10);
          ...
          console.log(
            `Admin password updated for ${adminEmail}, new hash: ${hashedPassword}`
          );
        }
      }
    } else {
