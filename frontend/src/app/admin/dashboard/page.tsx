...
export default function AdminDashboard() {
  ...
  const [metrics, setMetrics] = useState<Metrics>({ totalStudents: 0, totalTests: 0, totalResults: 0 });
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (!contentType || !contentType.includes('application/json')) {
            ...
            console.error('Non-JSON response for tests:', { status: res.status, text: text.slice(0, 100) });
    ...
    const fetchResults = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          if (!contentType || !contentType.includes('application/json')) {
            const text = await res.text();
            console.error('Non-JSON response for results:', { status: res.status, text: text.slice(0, 100) });
            throw new Error('Server returned a non-JSON response');
          }
          const data = await res.json();
    ...
    const fetchMetrics = async () => {
      try {
        ...
        const results = resultsRes.ok
          ...
          : [];
        console.log('Fetched metrics:', { students: students.length, tests: tests.length, results: results.length });
        setMetrics({
    ...
    const fetchQuestions = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          if (!contentType || !contentType.includes('application/json')) {
            const text = await res.text();
            console.error('Non-JSON response for questions:', { status: res.status, text: text.slice(0, 100) });
            throw new Error('Server returned a non-JSON response');
          }
          const data = await res.json();
    ...
    const fetchData = async () => {
      setIsLoading(true);
      await Promise.all([fetchTests(), fetchResults(), fetchMetrics(), fetchQuestions()]);
      setIsLoading(false);
    };
  ...
  const validateQuestion = () => {
    if (!currentQuestion.question || currentQuestion.options.some((opt) => !opt) || currentQuestion.correctAnswer < 0 || currentQuestion.correctAnswer >= currentQuestion.options.length) {
      setError('Please fill in all question fields and select a valid correct answer');
      return false;
    }
    return true;
  };
  ...
  const handleAddQuestion = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (!contentType || !contentType.includes('application/json')) {
          const text = await res.text();
          console.error('Non-JSON response for question creation:', { status: res.status, text: text.slice(0, 100) });
          throw new Error('Server returned a non-JSON response');
        }
        const data = await res.json();
      ...
      setAvailableQuestions([...availableQuestions, { ...currentQuestion, questionId: data.questionId }]);
      setCurrentQuestion({
        question: '',
        code: '',
        options: ['', '', '', ''],
  ...
  const handleCreateTest = async () => {
    if (!testName || !testDate || !testTime || !testDuration || questionIds.length === 0) {
      setError('All fields are required, and at least one question ID must be selected');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (!contentType || !contentType.includes('application/json')) {
          const text = await res.text();
          console.error('Non-JSON response for test creation:', { status: res.status, text: text.slice(0, 100) });
          throw new Error('Server returned a non-JSON response');
        }
        const data = await res.json();
      ...
      const newTest = { testId: data.testId, name: testName, date: isoDate, duration: Number(testDuration), questionIds };
      ...
      <div className="dashboard-container animate-slide-in">
        ...
        {activeSection === 'createTest' && (
          <div className="section create-test-section">
            ...
            <button onClick={handleAddQuestion} className="button-success" disabled={isLoading}>
              Add Question
            </button>
            ...
            <button onClick={handleCreateTest} className="gradient-button" disabled={isLoading || questionIds.length === 0}>
              Publish Test
            </button>
          </div>
        )}
