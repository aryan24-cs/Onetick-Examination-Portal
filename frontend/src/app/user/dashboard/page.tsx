...
export default function UserDashboard() {
    ...
    try {
      ...
      const fetchProfile = async () => {
        ...
        try {
          ...
          console.log('Profile response status:', { status: res.status, statusText: res.statusText });

          const contentType = res.headers.get('content-type');
          if (!contentType || !contentType.includes('application/json')) {
            const text = await res.text();
            console.error('Non-JSON response received for profile:', { status: res.status, text: text.slice(0, 100) });
      ...
      const fetchTests = async () => {
        ...
        try {
          ...
          console.log('Tests response status:', { status: res.status, statusText: res.statusText });

          const contentType = res.headers.get('content-type');
          if (!contentType || !contentType.includes('application/json')) {
            const text = await res.text();
            console.error('Non-JSON response received for tests:', { status: res.status, text: text.slice(0, 100) });
            throw new Error('Server returned a non-JSON response. Please check the API URL and server status.');
          }
          ...
          if (validTests.length === 0) {
            ...
            setError('No valid tests available. Please contact the administrator.');
          }
          ...
          console.log('Parsed tests:', parsedTests.map((t: Test) => ({ testId: t.testId, name: t.name, date: t.date.toString() })));
          setOngoingTests(parsedTests.filter((test: Test) => isTestOngoing(test)));
          setTests(parsedTests);
        } catch (err: any) {
          console.error('Fetch tests error:', err.message);
          setError(err.message || 'Error fetching tests. Please check if the backend server is running.');
        }
      };
      ...
      const fetchResults = async () => {
        ...
        try {
          ...
          console.log('Results response status:', { status: res.status, statusText: res.statusText });

          const contentType = res.headers.get('content-type');
          if (!contentType || !contentType.includes('application/json')) {
            const text = await res.text();
            console.error('Non-JSON response received for results:', { status: res.status, text: text.slice(0, 100) });
            throw new Error('Server returned a non-JSON response. Please check the API URL and server status.');
          }
  ...
  const handleTakeTestClick = (testId: string, testName: string) => {
    if (!testId || !isValidUUID(testId)) {
      console.error('Take Test button clicked with invalid testId:', { testId, testName });
      setError(`Cannot navigate to test: Invalid test ID ${testId}`);
      return;
    }
    const cleanTestId = testId.trim();
    console.log('Navigating to test:', { testId: cleanTestId, testName, timestamp: new Date().toISOString() });
      ...
      <div className="dashboard-container animate-slide-in">
                        ...
                        <tr key={test.testId} className="animate-slide-in">
                          ...
                          <td>
                            <button
                              ...
                              className={isTestActive(test) ? 'gradient-button' : 'gradient-button disabled'}
        ...
        {activeSection === 'profile' && (
          <div className="section">
            <h3>Profile</h3>
            {profile && (
              <div className="profile-details animate-slide-in">
