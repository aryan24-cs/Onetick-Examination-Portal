...
export default function UserDashboard() {
  ...
  const isTestActive = (test: Test) => {
    ...
    const testStart = new Date(test.date);
    const testEnd = new Date(testStart.getTime() + test.duration * 60 * 1000);
    const isActive = now >= testStart && now <= testEnd;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const fetchProfile = async () => {
        try {
          const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/student/profile/${studentId}`, {
      ...
      const fetchTests = async () => {
        try {
          ...
          const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/tests`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          if (res.ok) {
            const data = await res.json();
            ...
            console.log("Parsed tests:", parsedTests.map((t: Test) => ({ testId: t.testId, name: t.name, date: t.date.toISOString() })));
          ...
          setError("Error fetching tests. Please check if the backend server is running.");
        }
      };
      ...
      const fetchResults = async () => {
        try {
          ...
          const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/student/results/${studentId}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          if (res.ok) {
            const data = await res.json();
